...
def tile_spec_from_sbemimage(imgdir):
    print(f"Loading from {imgdir}")
    os.chdir(imgdir)
    if not os.path.exists("meta"):
        raise FileNotFoundError("Change to proper SBEMimage data directory!")
        ...
        for line in mdl:
            if line.startswith("TILE: "):
                ...
                if not os.path.exists(tile_spec["tile_file"]):
                    fnf_error = (
                        "Warning: File " + tile_spec["tile_file"] + " does not exist"
                    )
                    print(fnf_error)
    ...
    # only use the 1st stack, as for now the resolution parameters do not
...
def get_imgdir(data_dir, stack_name, run_num):
    return os.path.join(data_dir, f"{stack_name}_run{run_num:02d}")
...
def save_tile_id_map(outdir, tile_id_map, imgdir_list, section_range_list):
    ...
    section_range = dict(imgdir_list=imgdir_list, section_range_list=section_range_list)
...
def validate_section(tile_spec_list):
    ...
    if not increase_z.all():
        print(
            "Image list in valid! Image list should be "
            "sorted increasingly according to z."
        )
...
if __name__ == "__main__":
    ...
    outdir = os.path.join("some_out_dir", stack_name)
