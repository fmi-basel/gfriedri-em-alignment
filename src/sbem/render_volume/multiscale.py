...
async def open_scaled_view(
    base_path, base_scale_key, downsample_factors, downsample_method
):
    vscaled_spec = {
        "driver": "downsample",
        ...
        "base": {
            ...
            "kvstore": {"driver": "file", "path": base_path},
            "scale_metadata": {"key": base_scale_key},
        },
    }
...
async def create_scaled_volume(
    base_path,
    scale_key,
    size,
    chunk_size,
...
async def write_scaled_volume(
    ...
    vscaled = await open_scaled_view(
        base_path, base_scale_key, downsample_factors, downsample_method
    )
    ...
    scaled = await create_scaled_volume(
        base_path,
        scale_key,
        volume_size,
        chunk_size,
    ...
    hierarchy = SizeHiearchy(volume_size=volume_size, chunk_size=chunk_size)

    hierarchy.shard_size_in_chunks = (preshift_bits + minishard_bits) / 3
    hierarchy.shard_size = np.multiply(
        hierarchy.shard_size_in_chunks, hierarchy.chunk_size
    ).astype(int)
    hierarchy.grid_shape_in_shards = np.ceil(
        np.divide(hierarchy.volume_size, hierarchy.shard_size)
    ).astype(int)
    ...
    for i in range(num_shards[0]):
        for j in range(num_shards[1]):
            for k in range(num_shards[2]):
                ...
                box = get_shard_box(shard_index_xyz, hierarchy.shard_size, volume_size)
...
async def make_multiscale(
    ...
    if len(downsample_factors_list) != len(chunk_size_list):
        raise ValueError(
            "downsample_factors_list and chunk_size_list"
