...
async def render_volume(
    ...
    if not write_to_existing:
        volume, stitched_sections, xy_coords, size_hierarchy = await prepare_volume(
            load_sections_config,
            offset_dir,
            coord_file,
            volume_config,
        ...
        sections, write_xy_coords = load_sections_with_range(
            load_sections_config, write_range, coord_file
        )
    ...
    await write_volume(
        volume, write_sections, write_xy_coords, size_hierarchy, start_z=start_z
    )
