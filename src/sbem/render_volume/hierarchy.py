...
class SizeHierarchy:
    def __init__(self, volume_size, chunk_size=None, shard_size=None):
    ...
    def chunk_size(self, cs):
        ...
        if cs is None:
            ...
            self.grid_shape_in_chunks = np.ceil(
                np.divide(self.volume_size, self._chunk_size)
            )
            self.bits_xyz = np.ceil(
                np.log2(np.maximum(0, self.grid_shape_in_chunks - 1))
            )
    ...
    def shard_size(self, ss):
        ...
        if ss is None:
            ...
            self.grid_shape_in_shards = np.ceil(
                np.divide(self.volume_size, self.shard_size)
            ).astype(int)
    ...
    def compute_shard_size(self, preshift_bits, minishard_bits):
        ...
        self.shard_size = np.multiply(shard_size_in_chunks, self.chunk_size).astype(int)
