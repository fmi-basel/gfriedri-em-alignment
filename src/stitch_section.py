...
def get_tile_file(tile_id, sbem_dir, grid_num, section_num):
    ...
    grid_str = f"g{int(grid_num):04d}"
    tile_str = f"t{tile_id:04d}"
    section_str = f"s{int(section_num):05d}"
    filename = f"{stackname}_{grid_str}_{tile_str}_{section_str}.tif"
...
def main():
    ...
    # Load tile images.
    tile_map = load_image_to_tile_map(
        tile_id_map,
    ...
    fine_x, offsets_x = stitch_elastic.compute_flow_map(
        tile_map, cx, 0, stride=(stride, stride), batch_size=4
    )
    fine_y, offsets_y = stitch_elastic.compute_flow_map(
        tile_map, cy, 1, stride=(stride, stride), batch_size=4
    )
    ...
    config = mesh.IntegrationConfig(
        dt=0.001,
        gamma=0.0,
        k0=float(sofima_config["DEFAULT"]["k0"]),
        k=float(sofima_config["DEFAULT"]["k"]),
        stride=stride,
        num_iters=1000,
        max_iters=20000,
        stop_v_max=0.001,
    ...
    imsave(
        sofima_config["DEFAULT"]["outpath"] + f"_stitched_section{section_num:05d}.tif",
        stitched,
        compress=6,
        check_contrast=False,
    )
    imsave(
        sofima_config["DEFAULT"]["outpath"] + "_mask.tif",
        mask.astype(np.int8),
        compress=6,
        check_contrast=False,
    )
